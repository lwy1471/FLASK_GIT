...
def make_conversation():
	model_v1 = ConversationModel(Config.conversation_endpoint, Config.conversation_username, Config.conversation_password)
	return model_v1


def conversation_message(model_obj, workspace_id, message, context, alternate_intents):
	if not isinstance(model_obj, ConversationModel) :
...
def apiMessage():
	...
	if request.method == 'POST':
		try:
			...
			workspace_id = Config.workspace_main
			_, _, _, response = conversation_message(conversation_model_v1, \
					workspace_id, message, context, True)



			#response 확인해서 confidence level 확인하기
			if('action' in response['output']) and (response['output']['action']['call_discovery'] == "True"):
				print('Not in Conversation')
				response['output']['text'] = "Search the answer in Discovery..."
